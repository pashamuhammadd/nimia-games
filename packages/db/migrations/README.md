...
| `0018` | Seed 32 layanan katalog `/order` ke `services`, pakai uuid tetap yang sama dengan kode TypeScript-nya. Jalankan setelah `0017` |
| `0019` | RPC `accept_negotiation_offer` / `reject_negotiation_offer` — biar client bisa terima/tolak counter-offer staff sendiri, tanpa perlu melonggarkan `orders_update_admin_only` |
| `0020` | **Perbaikan keamanan (audit P0.5, 4 Agustus 2026)** — menutup 3 celah RLS terkait uang: (1) `order_negotiations.proposed_by` sekarang dipaksa oleh trigger berdasarkan role asli pemanggil, tidak bisa lagi dipalsukan jadi 'staff' oleh client; (2) transisi pembayaran client (`awaiting_payment` → `payment_submitted`) sekarang lewat RPC `submit_payment_transaction` yang menghitung ulang sendiri alamat wallet & jumlah yang diharapkan, bukan lewat UPDATE langsung yang dulu tidak mengunci kolom `final_price_usd`/`payment_expected_amount`; (3) `orders_insert_own` sekarang mengunci `status` dan semua kolom pembayaran/verifikasi harus `null` saat order baru dibuat. **Wajib dijalankan sebelum go-live** — lihat laporan audit untuk detail eksploitasinya. |
