...
// endpoint) that look like a memecoin, that ALSO have a real public
// presence (website + X/Twitter + at least one of Telegram/Discord) —
// that channel requirement was an explicit product decision (12 Agustus
// 2026), kept as-is here even after the 0-results tuning below. Projects
// like this commonly need launch/marketing animation (mascot/meme
// animation, logo reveal, GIF/sticker packs) even though none of them has
// explicitly asked for it — see tools/scoreFirmographic.ts for how that
// gap is scored and evidenced HONESTLY, as an inference, never as a
// stated request. `serviceFilter`/`audienceFilter` (DiscoveryParams)
// aren't applicable to this source — it's category-driven, not
// keyword-search-driven like Reddit.
//
// Tuning note (13 Agustus 2026 — first real run returned 0 candidates):
// two things made the original version too strict to ever realistically
// return anything —
//   1. It only checked the newest 20 coins from /coins/list/new. A
//      literally-brand-new listing (minutes/hours old) very often has NO
//      social links filled in yet at all — CoinGecko/the project team
//      populates those over the following days. Widened to MAX_DETAIL_CALLS
//      below and parallelized (DETAIL_CONCURRENCY) so scanning more coins
//      doesn't blow the serverless request's time budget.
//   2. It required a CoinGecko `categories` entry containing "meme" —
//      but a coin this fresh frequently has an EMPTY `categories` array;
//      CoinGecko's own team assigns categories after the fact, not at
//      listing time. detectMemeCategory() now also recognizes a few more
//      of CoinGecko's actual meme-adjacent category slugs, and
//      looksMemeByName() is a clearly-labeled FALLBACK for the
//      empty-categories case — it never claims a CoinGecko category that
//      doesn't exist; see how `memeCategory` is used below.
//
...
// scan for that here at all, only structured API fields (plus the one
// clearly-labeled name/symbol heuristic described above).
const MAX_DETAIL_CALLS = 40;
const DETAIL_CONCURRENCY = 10;

const MEME_CATEGORY_HINTS = ["meme", "dog-themed", "cat-themed", "elon musk", "parody"];
const MEME_NAME_HINTS = ["doge", "shib", "inu", "pepe", "wojak", "moon", "elon", "chad", "based", "frog", "cat", "meme"];

function detectMemeCategory(categories: string[]): string | null {
  return categories.find((c) => MEME_CATEGORY_HINTS.some((hint) => c.toLowerCase().includes(hint))) ?? null;
}

function looksMemeByName(name: string, symbol: string): boolean {
  const lower = `${name} ${symbol}`.toLowerCase();
  return MEME_NAME_HINTS.some((hint) => lower.includes(hint));
}
    ...
    for (let i = 0; i < toCheck.length && candidates.length < params.limit; i += DETAIL_CONCURRENCY) {
      ...
      const results = await Promise.all(
        batch.map(async (coin) => {
          try {
            const detail = (await coinGeckoFetch(`/coins/${coin.id}`, {
              localization: false,
              tickers: false,
              market_data: true,
      ...
      for (const entry of results) {
        ...
        const { coin, detail } = entry;

        const categories = (detail.categories ?? []).filter((c): c is string => Boolean(c));
        const memeCategory = detectMemeCategory(categories);
        let isMemeMatch = Boolean(memeCategory);
        if (!isMemeMatch && categories.length === 0 && looksMemeByName(coin.name, coin.symbol)) {
          // Not yet categorized by CoinGecko (common for very fresh
          // listings) — fall back to a name/symbol heuristic instead of
          // silently skipping every uncategorized coin. `memeCategory`
          // stays null so nothing downstream claims a CoinGecko category
          // that was never actually assigned.
          isMemeMatch = true;
        }
        if (!isMemeMatch) continue;
        ...
        const channelList = Object.entries(channels)
          ...
          .map(([k]) => k)
          .join(", ");
        const categoryLabel = memeCategory
          ? `"${memeCategory}"`
          : "meme-themed by name (not yet categorized by CoinGecko)";
        const text =
          `${coin.name} (${coin.symbol.toUpperCase()}) is a newly listed ${categoryLabel} project on CoinGecko` +
