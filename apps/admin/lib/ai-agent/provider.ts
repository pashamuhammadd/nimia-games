...
// lead's qualification write-up, an outreach draft's tone), never to
// decide a lead_score, a qualification_status, or to invent evidence.
// That split is deliberate: the brief requires the score to "remain
// explainable" (section 6) and evidence to never be invented (section
// 8) — both guarantees come from tools/score.ts's plain deterministic
// logic, which works identically with or without this provider
// configured. This file is the ONE place that calls out to a paid LLM
// API, so it's the one place that needs to fail gracefully per section
// 20 of the brief ("if a provider is not configured, the application
// must fail gracefully and explain what configuration is missing").
//
...
// "anthropic" is implemented in V1.
...
export function aiProviderStatusMessage(): string {
  ...
  return "AI_PROVIDER_API_KEY is not set — lead qualification, scoring, and outreach drafts use the built-in " +
    "deterministic engine only (no LLM call is made). Set AI_PROVIDER_API_KEY and AI_MODEL in .env.local to " +
    "enable AI-assisted prose.";
}
 ...
 * deterministic templates in tools/score.ts / outreach.ts rather than
 * breaking a pipeline run. */
export async function callAiProvider(params: {
  ...
  if (provider !== "anthropic") {
    return { configured: true, error: `AI_PROVIDER="${provider}" is not implemented in V1 (only "anthropic" is supported).` };
  }
