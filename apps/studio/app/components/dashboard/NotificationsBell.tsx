...
// In-app Notification Center bell (10 Agustus 2026) — replaces the static
// "You're all caught up, nothing new right now" placeholder Topbar.tsx has
// shipped with since the notifications phase first pass. Self-contained
// (owns its own open/closed + click-outside state) rather than threading
// bellOpen up into Topbar, so this can be dropped into any dashboard shell
// (apps/admin has its own near-identical copy — see that app's own
// NotificationsBell.tsx — with staff-facing copy/routes instead) without
// Topbar needing to know anything about notifications itself.
//
// No Supabase Realtime subscription — this app doesn't use Realtime
// anywhere yet, and the WRITE side (packages/db/migrations/0032_notifications.sql)
// is pure database triggers with no existing pub/sub wiring to hook into.
// Polling every 45s (plus an immediate refetch whenever the dropdown is
// opened) keeps this simple and consistent with how every other
// "did something change" surface in this dashboard already works (a
// server-rendered fetch on navigation, via revalidatePath calls in each
// action) — an upgrade to Realtime is a reasonable follow-up, not required
// for a working v1.
...
export function NotificationsBell({
  ...
  // Background poll, always running (not just while open) so the unread
  // badge itself stays fresh even with the dropdown closed.
  React.useEffect(() => {
