...
// Shared "open a Discord ticket" button (19 Agustus 2026 — extracted from
// this file's own one-off implementation so the "must connect Discord
// first" gate in createSupportTicketAction applies here too, not just to
// OrderDetail's newer per-order button). See DiscordTicketButton.tsx.
import { DiscordTicketButton } from "@/app/components/dashboard/DiscordTicketButton";
...
export function OrderWizard({ isAuthenticated }: OrderWizardProps) {
  ...
  if (wizard.submitted) {
    // "Discuss this brief on Discord" (18 Agustus 2026, per user request)
    // — lets a client jump straight from the confirmation screen into a
    // Discord ticket about the brief they just submitted, instead of
    // navigating to /dashboard/support and retyping it. Same ORD-XXXXXXXX
    // short form submitOrderAction/submitCustomOrderAction already use for
    // their own success messages — re-derived here from the raw uuid
    // wizard.submittedOrderId holds, rather than adding a new column just
    // for display. DiscordTicketButton (19 Agustus 2026) now owns the
    // actual click/status/connect-gate handling — this only builds the
    // subject/message text, which is specific to this screen.
    ...
    const discordSubject = `Let's discuss my brief${projectTitle ? ` — ${projectTitle}` : ""}`.slice(0, 120);
        ...
        <main className="mx-auto flex max-w-xl flex-col items-center px-4 py-24 text-center sm:px-6">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            ...
            <DiscordTicketButton
              ...
              orderId={wizard.submittedOrderId ?? undefined}
              label="Discuss this brief on Discord"
            />
          </div>
