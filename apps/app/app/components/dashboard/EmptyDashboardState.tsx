...
// Shown on first login / before a client has submitted any order at all
// (page.tsx checks: zero rows in both `orders` and `projects` for this
// client). Replaces the stat cards + Active Orders + Recent Activity
// entirely, per the brief's "Empty State" section — the greeting banner
// above it still shows.
export function EmptyDashboardState({ ctaHref }: { ctaHref: string }) {
  return (
    <motion.div
    ...
    >
      <div className="relative flex h-24 w-24 items-center justify-center">
        ...
        <div className="relative flex h-20 w-20 items-center justify-center rounded-full border border-white/10 bg-white/[0.04]">
          <Rocket className="h-9 w-9 text-[var(--nimia-pink)]" aria-hidden="true" />
          <Sparkles
      ...
      <div className="flex flex-col gap-2">
        <h2 className="text-lg font-semibold text-white sm:text-xl">No Projects Yet</h2>
        <p className="max-w-sm text-sm text-white/45">
          Start your first project with Nimia Studio and track everything from one dashboard.
        </p>
      </div>
